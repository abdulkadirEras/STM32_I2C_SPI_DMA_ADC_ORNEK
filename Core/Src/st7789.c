...
static void ST7789_WriteData(uint8_t *buff, size_t buff_size)
{
	...
	while (buff_size > 0) {
		uint16_t chunk_size = buff_size > 65535 ? 65535 : buff_size;
			...
			{
				if(HAL_SPI_Transmit(&ST7789_SPI_PORT, buff, chunk_size, HAL_MAX_DELAY)!=HAL_OK)
					Error_Handler();
			}
		#else
			if(HAL_SPI_Transmit(&ST7789_SPI_PORT, buff, chunk_size, HAL_MAX_DELAY)!=HAL_OK)
...
void ST7789_Fill_Color(uint16_t color)
{
	...
	ST7789_Select();
		...
		for (i = 0; i < ST7789_HEIGHT / HOR_LEN; i++)
		{
			...
			ST7789_WriteData(disp_buf, sizeof(disp_buf));//ORJİNALİ ST7789_WriteData(disp_buf, sizeof(disp_buf));
		}
	#else
		uint16_t j;
...
void ST7789_Test(void)
{
	...
	HAL_Delay(1000);
	ST7789_WriteString(10, 20, "Hiz Test", Font_11x18, RED, WHITE);
	HAL_Delay(1000);
	...
	ST7789_WriteString(10, 50, "Selam Kadir!", Font_7x10, RED, WHITE);
	ST7789_WriteString(10, 75, "Selam Kadir!", Font_11x18, YELLOW, WHITE);
	ST7789_WriteString(10, 100, "Selam Kadir!", Font_16x26, MAGENTA, WHITE);
	HAL_Delay(1000);

	ST7789_Fill_Color(RED);
	ST7789_WriteString(10, 10, "Dik./Ciz.", Font_11x18, YELLOW, BLACK);
	ST7789_DrawRectangle(30, 30, 100, 100, WHITE);
	HAL_Delay(1000);

	ST7789_Fill_Color(RED);
	ST7789_WriteString(10, 10, "Dolu Dik.", Font_11x18, YELLOW, BLACK);
	ST7789_DrawFilledRectangle(30, 30, 50, 50, WHITE);
	HAL_Delay(1000);

	ST7789_Fill_Color(RED);
	ST7789_WriteString(10, 10, "Daire.", Font_11x18, YELLOW, BLACK);
	ST7789_DrawCircle(60, 60, 25, WHITE);
	HAL_Delay(1000);

	ST7789_Fill_Color(RED);
	ST7789_WriteString(10, 10, "Dolu daire.", Font_11x18, YELLOW, BLACK);
	ST7789_DrawFilledCircle(60, 60, 25, WHITE);
	HAL_Delay(1000);

	ST7789_Fill_Color(RED);
	ST7789_WriteString(10, 10, "UC", Font_11x18, YELLOW, BLACK);
